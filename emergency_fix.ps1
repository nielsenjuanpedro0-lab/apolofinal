$raw = Get-Content -Path "src\App.tsx" -Raw -Encoding UTF8

# Define the clean replacement for the boundary and the Financiacion component
$replacement = @"
              </div>
            </motion.div>
          ))}
...
# We need to find the start of the garbage and the end.
# Start of garbage is the line with "Ver Detalles" + "</div>const Financiacion"
# End of garbage is right before "const Nosotros"
...
if ($startLineIdx -eq -1 -or $endLineIdx -eq -1) {
    Write-Error "Could not find start or end markers"
    exit 1
}

# The line at $startLineIdx looks like: <Link ...>Ver Detalles</Link></div>const Financiacion...
# We want to keep everything till "</Link>" and then start our replacement
$fixStartLine = $lines[$startLineIdx] -replace "Ver Detalles</Link>.*", "Ver Detalles</Link>"

$newContent = $lines[0..($startLineIdx-1)] + $fixStartLine + $replacement + $lines[$endLineIdx..($lines.Length-1)]

$newContent | Set-Content -Path "src\App.tsx" -Encoding UTF8
